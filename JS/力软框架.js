...
function Loading(bool, text) {
    ...
    if (!!text) {
        ...
        top.$("#loading").css("left", (top.$('body').width() - top.$("#loading").width()) / 2 - 20);
    } else {
        top.$("#loading span").html("正在加载…");
...
请求Ajax 带返回值
*/
function getAjaxAsync(url, postData, callBack) {
    $.ajax({
        type: 'post',
        dataType: "json",
        url: RootPath() + url,
...
弹出对话框（带：确认按钮、取消按钮）
*/
function openDialog_SecAera(url, _id, _title, _width, _height, callBack, callBack1) {
    Loading(true);
    top.$.dialog({
        id: _id,
        width: _width,
...
/*
弹出对话框（带：确认按钮、取消按钮sumbit）
*/
function openDialog_Submit(url, _id, _title, _width, _height, callBack, callBack1) {
    Loading(true);
    top.$.dialog({
        id: _id,
        width: _width,
...
function autocomplete(Objkey, width, height, data, callBack) {
    ...
    if (data != "") {
        ...
        sbhtml += '<tbody><tr><td style="color:red;text-align:center;width:auto;">没有找到您要的相关数据！</td></tr></tbody>';
    }
    sbhtml += '</table>';
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' ondblclick=\"RemoveDiv('" + tabid + "')\" win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a><img src='../Content/Images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
...
//生成导出文件
function ExportFile(title, url) {
    Loading(true, title);
    //加载内容
    top.$('#exportframe').attr('src', url);
...
function InitDropByUrl(url, ControlId, postData) {
    $(ControlId).html("");
    ...
    $.ajax({
        ...
        success: function (DataJson) {
            $.each(DataJson, function (i) {
                $(ControlId).append("<option value='" + DataJson[i].code + "'>" + DataJson[i].name + "</option>");
            });
        }
    });
}
